...
class Menuebarpaym extends StatefulWidget {
final int price;  
final int course_id;  
const Menuebarpaym({super.key,required this.price,required this.course_id});

  @override
  State<Menuebarpaym> createState() => _MenuebarpaymState();
}

class _MenuebarpaymState extends State<Menuebarpaym> {
bool valuetoggle=false;
String ?chargeCard="كارت شحن أشطر";
String ?fawryCard="fawrypay";
...
Text('الترم التاني [الصف الثالث الثانوي]')
  ,  
    ...
    Image.network(
                  'https://media.githubusercontent.com/media/hagartarek-design/webflutter/refs/heads/main/assets/logo1.png',
                  height: 40,width: 40,
                ),        ],)
            ],),),
 ...
 Container(
  ...
  width: 343, child :Text( 'طرق الدفع' ,textAlign: TextAlign.end,style: TextStyle( fontWeight: FontWeight.w600,fontSize: 24,color: Color.fromARGB(255, 6, 69, 152)))
     ...
     Container(width: 311,height: 51, child: Row(children: [
     
 Container(width: 64,height: 51,
 child: Column(children: [Text('${controller.profileData!['balance']} ج .م')
 , Container(width: 39,height: 20,child: DraggableToggleSwitch(//isToggled: valuetoggle,
                    initialValue:false ,
...
,
        ...
        SizedBox(height: 24,),
          Row(children: [ Container(width: 311,//height: 178,
        child: 
  ...
  SizedBox(height: 24,),
  Container(width: 311,child:Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
...
SizedBox(height: 24,)
,Container( width:311,height: 44 ,child: TextButton(onPressed:(){
controller.fetch();
  controller.purchasecourse(courseId:widget.course_id);
}, child: Text('ادفع'
